...
cc.Class({
    ...
    {
        // joy下的Game组件
        this._joyCom = this.node.parent.getComponent('controlKey');
        // game组件下的player节点
   ...
   update: function(dt)
    ...
    _allDirectionsMove: function()
    {
        ...
        if(this._playerNode.x > this.node.parent.parent.width/2){
            this._playerNode.x = this.node.parent.parent.width/2;
        }else if(this._playerNode.x < -this.node.parent.parent.width/2){
            this._playerNode.x = -this.node.parent.parent.width/2;
        }

        if(this._playerNode.y > this.node.parent.parent.height/2)
        {
            this._playerNode.y = this.node.parent.parent.height/2;
        }else if(this._playerNode.y < -this.node.parent.parent.height/2){
            this._playerNode.y = -this.node.parent.parent.height/2;
        }
    ...
    speedUp(){
        ...
        this.schedule(this.speedDown,0.3,0)
    },

    speedDown(){
        this.speed2 = this._speed2;
    }
});
